...
def ProgressProcess():
  global FE_PROGRESS, PP_PROGRESS, CL_PROGRESS
...
def featuresExtraction():
  ...
  if (request.method == "POST"):
    ...
    # storagePath = os.path.join(app.config["FEATURES_FOLDER"], f"{pickleFile.split('.')[0]}_{distances}")
    ...
    #   f"{pickleFile.split('.')[0]}_{currentDT}"
    # )
    # os.makedirs(storagePath, exist_ok=True)
      ...
      try:
        for j, c in enumerate(cnts):
          ...
          for q, quantile in enumerate(quantiles):
            ...
            rndChoice = None
            roiAug, tempAug = None, None
            data = [(roi, temp)]

            if (len(augmentationTechsList) > 0):
              rndChoice = random.choice(augmentationTechsList)
              if (rndChoice == "rotate"):
                roiAug, tempAug = RandomRotation(roi, temp)
              elif (rndChoice == "horz"):
                roiAug, tempAug = HorizontalFlip(roi, temp)
              elif (rndChoice == "vert"):
                roiAug, tempAug = VerticalFlip(roi, temp)
              elif (rndChoice == "zoom"):
                roiAug, tempAug = RandomZoom(roi, temp)
              elif (rndChoice == "shift"):
                roiAug, tempAug = RandomTranslate(roi, temp)
              else:
                print("Invalid augmentation technique.")
...
def classification():
  ...
  if (request.method == "POST"):
    ...
    for fileKeyword in ["First_Stage", "Second_Stage_Up", "Second_Stage_Down"]:
      ...
      top1Metrics[columns2PrintIndividual[2:]] = top1Metrics[columns2PrintIndividual[2:]].applymap(lambda x: f"{x:.2f}")
      # Add "Order" column.
      ...
      # Save the top-1 metrics to a CSV file.
      top1MetricsComb.to_csv(os.path.join(clsFolder, f"{fileKeyword}_Top1_Top1_Combinations_Metrics.csv"), index=False)
    ...
    with open(os.path.join(clsFolder, "Configurations.pickle"), "wb") as f:
      pickle.dump(configurations, f)
